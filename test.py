...
def get_nodes_and_metrics(account):
    ...
    try:
        ...
        for node in node_info:
            ...
            # If node_name includes '/', extract the last part
            if '/' in node_name:
                original_node_name = node_name
                node_name = node_name.split('/')[-1]
                print(f"Adjusted node_name from '{original_node_name}' to '{node_name}'")
    ...
    try:
        ...
        for pod in pod_info:
            try:
                ...
                # If pod_name includes '/', extract the last part
                if '/' in pod_name:
                    original_pod_name = pod_name
                    pod_name = pod_name.split('/')[-1]
                    print(f"Adjusted pod_name from '{original_pod_name}' to '{pod_name}'")
