...
# Define namespace suffixes for each environment
NAMESPACE_SUFFIX_MAPPING = {
    'dev': ['dev', 'devb', 'devc'],
    'intg': ['intgb', 'intgc'],
    'prod': ['proda', 'prodp'],
...
def get_namespace_group(namespace, environment):
    ...
    """
    suffixes = NAMESPACE_SUFFIX_MAPPING.get(environment, [])
...
def group_namespaces_by_suffix(namespaces, environment):
    """Group namespaces by their suffix based on the environment."""
    grouped_namespaces = {}
    suffixes = NAMESPACE_SUFFIX_MAPPING.get(environment, [])

    if environment == 'idev':
        # All namespaces are in a single group
        grouped_namespaces['All'] = list(namespaces)
    else:
        for suffix in suffixes:
            grouped_namespaces[suffix] = [ns for ns in namespaces if ns.endswith(suffix)]
        # Optionally, add a group for namespaces that don't match any suffix
        other_namespaces = [ns for ns in namespaces if not any(ns.endswith(suffix) for suffix in suffixes)]
        if other_namespaces:
            grouped_namespaces['other'] = other_namespaces
...
def generate_html_report(clusters_info, current_env):
    ...
    <head>
        ...
        <style>
            ...
            .download-link { margin-top: 20px; display: inline-block; }
            .gauge-container { width: 100px; height: 20px; background: #e6e6e6; border-radius: 5px; position: relative; }
            .gauge-fill { height: 100%; border-radius: 5px; }
            .gauge-label { position: absolute; top: 0; left: 50%; transform: translateX(-50%); font-weight: bold; font-size: 12px; }
    ...
    # Group namespaces by suffix for each cluster based on the current environment
    for cluster in clusters_info:
        grouped_namespaces = group_namespaces_by_suffix(cluster['pods'].keys(), cluster['account'])
    ...
    for account in accounts:
        ...
        for cluster in clusters:
            ...
            clusters_info.append({
                ...
                'nodes': nodes,
                'pods_info': pods_info,
                'pods': namespace_counts,  # Pass namespace counts to the template
