        ...
        <script>
            ...
            function showPage(page, rows) {
                ...
                for (let i = 0; i < rows.length; i++) {
                    rows[i].style.display = (i >= start and i < end) ? '' : 'none';
                }
            }
        </script>
    </body>
...
if __name__ == '__main__':
    ...
    environments = ['dev', 'devB', 'devC']  # List of environments to process

    all_clusters_info = []
    for environment in environments:
        print(f"Processing environment: {environment}")
        result = lambda_handler({'queryStringParameters': {'environment': environment}}, None)
        all_clusters_info.append(result['body'])

    # Combine all results or do something with them
    with open('eks_dashboard_all.html', 'w') as f:
        f.write("\n".join(all_clusters_info))
    print("Combined Dashboard HTML generated.")
